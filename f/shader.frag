...
/*
    ...
    Desert Minecart Track Ride by runevision
    ----------------------------------------
    - Added minecart track.
    - Added underground caves.
    - Implemented camera speed accelleration based on slope.
    - Made camera and path swiveling depend on camera speed.
    - Decreased amount camera looks up/down to follow path, to give greater sense of going up/down.
    - Added screen shake when turning while going fast.
    - Adjusted path to be more dramatic.
...
// Tracer from https://www.shadertoy.com/view/7tBfzz
// Tweaked values performance/quality balance optimized for this scene.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord){
    ...
    if (t < FAR){
        ...
        sn = doBumpMap(iChannel1, sp*tSize1, sn, .007/((1. + t/FAR) * (1.0 + svObjID)));//max(1.-length(fwidth(sn)), .001)*hash(sp)/(1.+t/FAR)
        ...
        float spe = pow(max( dot( reflect(-ld, sn), -rd ), 0. ), 5.* (svObjID + 1.0)); // Specular term.
		...
		float fre2 = mix(.2 * (svObjID + 1.0), 1., Schlick);  //F0 = .2 - Hard clay... or close enough.
