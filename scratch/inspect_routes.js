...
async function run() {
  try {
    const res = await databases.listDocuments(dbId, 'routes');
    console.log('Total documents:', res.documents.length);
    res.documents.forEach(doc => {
      console.log(`- ID: ${doc.$id}, Name: ${doc.name}, Stops:`, doc.stops);
    });
  } catch (err) {
